...
# update.sh - InstaHack
#
# The shell / bash script that updates the InstaHack tool that had already been set up in your computer system. It just fetches te newer version of the project's repository at the github mirror (https://github.com/sslri/instahack/). The script can be launched directly, but the codes are written in such a way that the script can only be used inside the main script file for the tool i.e., 'instahack.sh'.
#
# Author : Saeed Salari (https://github.com/sslri/)
...
if wget --spider https://raw.githubusercontent.com/sslri/instahack/master/update.v1.4 2>/dev/null; then
	...
	if [[ $choice == "y" ]]; then
		...
		while git clone https://github.com/sslri/instahack 2> /dev/null; do 
			printf  "\e[92m▓▓▓▓▓▓▓▓▓▓▓▓▓\e[0m"
			sleep 0.5
		done
